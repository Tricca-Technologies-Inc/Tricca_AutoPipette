...
def transferLiq(source, dest, volume, pipette):
    for _ in range(volume.multiplier):
        ...
        pipette.dipZ(source, AutoPipette.VIAL_DIP_DISTANCE)
        pipette.home_stepper(speed=30)
        pipette.returnZ(source, AutoPipette.VIAL_DIP_DISTANCE)

        # Dispense
        dest.speed = AutoPipette.DEFAULT_SPEED
        pipette.move_to(dest)
        pipette.dipZ(dest, AutoPipette.TILTV_DIP)
        pipette.move_stepper(volume.dispense)
        pipette.returnZ(dest, AutoPipette.TILTV_DIP)
        pipette.home_stepper(speed=30)
