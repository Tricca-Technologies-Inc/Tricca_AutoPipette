...
import argparse
import requests
from autopipette_shell import AutoPipetteShell


GCODE_PATH = Path(__file__).parent.parent / 'gcode/'
URL_MOONRAKER = "http://0.0.0.0:7125/printer/gcode/script"


def main():
    """Entry for the program if run through terminal."""
    ...
    parser.add_argument("-t",
                        "--term",
                        help="The ip address of the auto-pipette.",
                        action="store_true")
    args = parser.parse_args()
    # Launch program
    # pipette = AutoPipette()
    global URL_MOONRAKER
    URL_MOONRAKER = \
...
def send_gcode(command):
    ...
    response = requests.post(URL_MOONRAKER, json={"script": command})
